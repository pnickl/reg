...
class MultiGPRegressor(gpytorch.models.ExactGP):
	...
	def __init__(self, input, target, likelihood, dm_target):
		super(MultiGPRegressor, self).__init__(input, target, likelihood)

		self.input = input
		self.target = target
		self.dm_target = dm_target

		self.mean_module = gpytorch.means.MultitaskMean(gpytorch.means.ConstantMean(), num_tasks=dm_target)
		self.covar_module = gpytorch.kernels.MultitaskKernel(gpytorch.kernels.RBFKernel(), num_tasks=dm_target, rank=1)
